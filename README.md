...
- **Thread history** - when a session can't be resumed, the bridge fetches full thread history as context
...
- **No bot token needed** - reuses Claude Code's Slack MCP OAuth token

### Reliability
- **Startup safety** - ignores all pre-existing messages on startup
- **Graceful shutdown** - handles SIGINT/SIGTERM cleanly
- **Auto-recovery** - re-authenticates Gmail on token expiry, re-reads Slack token each cycle
- **Token expiry handling** - Slack token refreshed when you run any Slack MCP tool in Claude Code
...
├── slack_agent_state.json  # Slack channel ID, last checked ts, active threads
...
Edit constants at the top of `bridge.py`, or use environment variables:
